...
def print_time(time):
    ...
    print('{%.2d}'(time.hour, time.minute, time.second))
...
def increment_2(time, seconds):
    ...
    result = Time(
        result.hour, result.minute, result.second = time.hour
    )
...
def mul_time(t1, factor):
    ...
    return time.int_to_time(int(seconds))
